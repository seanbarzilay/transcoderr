...
impl Engine {
    ...
    pub async fn run(&self, flow: &Flow, job_id: i64, mut ctx: Context) -> anyhow::Result<Outcome> {
        ...
        match self.run_nodes(&flow.steps, job_id, &mut ctx, &mut counter, resume).await {
            ...
            Err(e) => {
                db::run_events::append_with_bus(&self.pool, &self.bus, job_id, None, "failed",
                ...
                match n {
                    Node::Step { id, use_, with, retry } => {
                        ...
                        for attempt in 1..=max_attempts {
                            db::run_events::append_with_bus(&self.pool, &self.bus, job_id, Some(&step_id), "started",
                            ...
                            let mut cb = move |ev: StepProgress| {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    let _ = db::run_events::append_with_bus(&pool, &bus, job_id, Some(&step_id), &kind, Some(&payload)).await;
                                });
                            };
                    ...
                    Node::Conditional { id, if_, then_, else_ } => {
                        ...
                        db::run_events::append_with_bus(&self.pool, &self.bus, job_id, Some(&step_id), "condition_evaluated",
                    ...
                    Node::Return { return_ } => {
                        db::run_events::append_with_bus(&self.pool, &self.bus, job_id, None, "returned",
