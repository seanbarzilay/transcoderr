...
impl Server {
    #[tool(
        name = "list_sources",
        description = "List webhook sources (radarr/sonarr/lidarr/generic). Secret tokens are redacted to `***` in the response (token-authed callers can't recover them)."
    )]
    pub async fn list_sources(
        &self,
        _: Parameters<super::NoArgs>,
    ) -> Result<Json<Vec<SourceSummary>>, ErrorData> {
        self.api
    ...
    #[tool(
        name = "get_source",
        description = "Get a source by id. Secret token is redacted in the response."
    )]
    ...
    #[tool(
        name = "create_source",
        description = "Create a new webhook source. Side effect: registers a new endpoint that *arr instances can hit. `kind` is one of `radarr|sonarr|lidarr|generic`; `secret_token` is what the *arr will use for Bearer or Basic auth."
    )]
    ...
    #[tool(
        name = "update_source",
        description = "Patch fields on an existing source. Side effect: changes incoming-webhook auth or routing. Omitted fields are unchanged. Sending `\"***\"` for secret_token is treated as 'unchanged'."
    )]
    ...
    #[tool(
        name = "delete_source",
        description = "Destructive: permanently delete a source. Subsequent webhooks for this source will be rejected with 401. Cannot be undone. Requires confirm=true."
    )]
