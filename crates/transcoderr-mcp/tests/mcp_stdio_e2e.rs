...
"#, data_dir.display())).unwrap();
    ...
    let bin_dir = std::path::Path::new(mcp_bin).parent().expect("mcp bin parent");
    let server_bin = bin_dir.join(if cfg!(windows) { "transcoderr.exe" } else { "transcoderr" });
    assert!(server_bin.exists(), "transcoderr binary not found at {server_bin:?}; run `cargo build -p transcoderr` first");
    ...
    assert!(init["result"]["serverInfo"]["name"].as_str().is_some(), "init result: {init}");
    ...
    assert!(runs.get("error").is_none() || runs["error"].is_null(), "list_runs failed: {runs}");
    ...
    let txt = created["result"]["content"][0]["text"].as_str()
        .unwrap_or_else(|| panic!("create_flow missing content[0].text: {created}"));
    let payload: serde_json::Value = serde_json::from_str(txt)
        .unwrap_or_else(|_| panic!("create_flow content not JSON: {txt}"));
    assert!(payload["id"].as_i64().is_some(), "create_flow payload missing id: {payload}");
    ...
    assert!(dry.get("error").is_none() || dry["error"].is_null(), "dry_run_flow failed: {dry}");
...
fn seed_api_token(db_path: &std::path::Path, raw_token: &str) {
    ...
    rt.block_on(async {
        ...
        let pool = SqlitePoolOptions::new()
            .connect(&url).await.expect("connect to seeded sqlite");
        ...
        sqlx::query("INSERT INTO api_tokens (name, hash, prefix, created_at) VALUES (?, ?, ?, ?)")
            ...
            .execute(&pool).await.expect("insert api_token");
        // Enable auth so require_auth runs.
        sqlx::query("INSERT OR REPLACE INTO settings (key, value) VALUES ('auth.enabled','true')")
            .execute(&pool).await.expect("set auth.enabled");
        let pw_hash = transcoderr::api::auth::hash_password("unused").expect("hash placeholder password");
        sqlx::query("INSERT OR REPLACE INTO settings (key, value) VALUES ('auth.password_hash', ?)")
            .bind(pw_hash).execute(&pool).await.expect("set auth.password_hash");
    });
}
