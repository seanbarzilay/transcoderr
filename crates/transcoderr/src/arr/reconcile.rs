...
async fn run(pool: &SqlitePool, public_url: &str) -> anyhow::Result<()> {
    ...
    for src in sources {
        let Some(arr_kind) = arr::Kind::parse(&src.kind) else {
            continue;
        };
        ...
        let Some(base_url) = cfg.get("base_url").and_then(|v| v.as_str()) else {
            continue;
        };
        let Some(api_key) = cfg.get("api_key").and_then(|v| v.as_str()) else {
            continue;
        };
...
async fn reconcile_one(
    ...
    match client.get_notification(notification_id).await? {
        ...
        Some(_) => {
            ...
            tracing::info!(
                source_id = src.id,
                old_id = notification_id,
                new_id = new_n.id,
                "*arr webhook recreated"
            );
        }
        None => {
            ...
            if let Some(found) = existing {
                ...
                tracing::warn!(
                    source_id = src.id,
                    missing_id = notification_id,
                    "*arr webhook missing; recreating"
                );
                let new_n = client
                    .create_notification(arr_kind, &src.name, &expected_url, &src.secret_token)
                    .await?;
