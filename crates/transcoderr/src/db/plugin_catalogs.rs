...
pub async fn record_fetch_success(pool: &SqlitePool, id: i64) -> sqlx::Result<()> {
    let now = chrono::Utc::now().timestamp();
    sqlx::query("UPDATE plugin_catalogs SET last_fetched_at = ?, last_error = NULL WHERE id = ?")
        .bind(now)
        .bind(id)
        .execute(pool)
        .await?;
...
pub async fn record_fetch_error(pool: &SqlitePool, id: i64, err: &str) -> sqlx::Result<()> {
    sqlx::query("UPDATE plugin_catalogs SET last_error = ? WHERE id = ?")
        .bind(err)
        .bind(id)
        .execute(pool)
        .await?;
...
mod tests {
    ...
    async fn create_then_list() {
        ...
        let id = create(
            &pool,
            "internal",
            "https://internal.example/index.json",
            Some("Bearer xyz"),
            5,
        )
        .await
