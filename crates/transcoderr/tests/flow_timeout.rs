...
async fn shell_step_timeout_fails_quickly() {
    ...
    transcoderr::steps::registry::init(
        pool.clone(),
        transcoderr::hw::semaphores::DeviceRegistry::empty(),
...
"#;
    ...
    let job_id = db::jobs::insert(&pool, flow_id, 1, "radarr", "/x", "{}")
        .await
        .unwrap();
    ...
    let outcome = Engine::new(pool.clone(), bus, dir.path().to_path_buf())
        .run(&flow, job_id, Context::for_file("/x"))
        .await
        .unwrap();
    ...
    assert!(
        elapsed.as_secs() < 3,
        "timeout should fire within ~1-2s, took {:?}",
        elapsed
    );
    ...
    assert!(
        ...
        "expected timeout in payload: {payload}"
    );
}
