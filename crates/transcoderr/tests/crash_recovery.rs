...
"#;
    ...
    transcoderr::steps::dispatch("probe")
        .unwrap()
        .execute(&Default::default(), &mut ctx, &mut |_| {})
        .await
        .unwrap();
    db::checkpoints::upsert(&pool, job_id, 0, &ctx.to_snapshot())
        .await
        .unwrap();
    ...
    let outcome = Engine::new(pool.clone(), bus, dir.path().join("db"))
        .run(&flow, job_id, Context::for_file(movie.to_string_lossy()))
        .await
        .unwrap();
    ...
    let evts: Vec<(String,)> =
        sqlx::query_as("SELECT kind FROM run_events WHERE job_id = ? AND step_id = 'probe'")
            .bind(job_id)
            .fetch_all(&pool)
            .await
            .unwrap();
    assert_eq!(
        evts.len(),
        0,
        "probe should have been skipped due to checkpoint resume"
    );
}
